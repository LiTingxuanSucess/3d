...
class ShapeNet_test(data.Dataset):
    ...
    def __init__(self, opt, train=True):
        ...
        if not opt.demo:
            ...
            # print("dir_pointcloud")
            print(dir_pointcloud)
            dir_image = join(self.image_path, category)
            # print(dir_image)
            list_pointcloud = sorted(os.listdir(dir_pointcloud))
            # print(list_pointcloud)
            if self.train:
                list_pointcloud = list_pointcloud[:int(len(list_pointcloud) * 1)]
            else:
                list_pointcloud = list_pointcloud[int(len(list_pointcloud) * 0.8):int(len(list_pointcloud))]
            print(
                '    category '
            ...
            for pointcloud in self.category_datapath[category]:
    ...
    def __getitem__(self, index):
        ...
        if self.opt.SVR:
            if self.train:
                ...
                im = Image.open(join(return_dict['image_path'], ShapeNet_test.int2str(N) + ".png"))
                im = self.dataAugmentation(im)  # random crop
            else:
                im = Image.open(join(return_dict['image_path'], ShapeNet_test.int2str(self.idx_image_val) + ".png"))
    ...
    def __init__(self, opt, train=True):
        ...
        if not opt.demo:
            ...
            for category in self.classes:
                ...
                list_pointcloud = sorted(os.listdir(dir_pointcloud))
                if self.train:
                    list_pointcloud = list_pointcloud[:int(len(list_pointcloud) * 0.9)]
                else:
                    list_pointcloud = list_pointcloud[int(len(list_pointcloud) * 0.5):]
                print(
                    '    category '
                    + colored(category, "yellow")
                    + "  "
...
if __name__ == '__main__':
    ...
    opt = {"normalization": "UnitBall", "class_choice": ["plane"], "SVR": True, "sample": True, "npoints": 2500,
           "shapenet13": True}
    d = ShapeNet_test(EasyDict(opt), train=False, keep_track=True)
    print(d[1])
    a = len(d)
